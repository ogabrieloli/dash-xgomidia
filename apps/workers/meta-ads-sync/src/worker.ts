    ...
    for (const metric of metrics) {
      ...
      await db.metricSnapshot.upsert({
        ...
        create: {
          ...
          initiateCheckout: metric.initiateCheckout ?? 0,
          viewContent: metric.viewContent ?? 0,
          completeRegistration: metric.completeRegistration ?? 0,
          postEngagement: metric.postEngagement ?? 0,
          videoViews3s: metric.videoViews3s ?? 0,
          linkClicks: metric.linkClicks ?? 0,
          landingPageViews: metric.landingPageViews ?? 0,
          profileVisits: metric.profileVisits ?? 0,
          pageEngagement: metric.pageEngagement ?? 0,
